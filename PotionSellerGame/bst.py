...
class BinarySearchTree(Generic[K, I]):
    ...
    def get_successor(self, current: TreeNode) -> TreeNode:
        ...
        author: Mah Ying Qi
            ...
            larger keys.
            Search the minimal node in right sub-tree
            :precondition: current should be a tree node
            :complexity: O(log(N)) N is total number of nodes, as maximum length we will traverse is log(N)
            :post condition: tree is not modified
            :invariant: successor is always greater than current if it is not None
            :return The smallest node greater than the current
        """
        successor = current.right
        if successor is not None:
            # Return none if there is no successor, else return the minimum node in the right subtree
            successor = self.get_minimal(successor)
        return successor
    ...
    def get_minimal(self, current: TreeNode) -> TreeNode:
        ...
        author: Mah Ying Qi
            ...
            Traverse down to the end of the leftmost path
            :precondition: current should be a tree node
            :complexity O(log(N)) N is the number of nodes in the subtree, as maximum length we will traverse is log(N)
            :post condition: tree is not modified
            :invariant: current is greater or equals to minimal node
