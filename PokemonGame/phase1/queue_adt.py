...
class CircularQueue(Queue[T]):
    ...
    def __str__(self)->str:
        """
        Method to present queue as a string
        :complexity: Best == Worst O(n), n is size of queue
        :return: all values of item in queue as string
        """
        res = ""
        for i in range(len(self)):
            index = (self.front + i) % len(self.array)
